...
CREATE TABLE guilds (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (leader_id) REFERENCES players(player_id) ON DELETE SET NULL,
    FOREIGN KEY (game_id) REFERENCES games(game_id) ON DELETE CASCADE
);
...
CREATE TABLE guild_members (
    ...
    UNIQUE KEY unique_guild_member (guild_id, player_id),
...
CREATE TABLE daily_player_stats (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_player_date (player_id, stat_date),
    FOREIGN KEY (player_id) REFERENCES players(player_id) ON DELETE CASCADE
);
...
-- Completion message
-- ===========================================
SELECT 'GameVerse database initialized successfully!' AS message;
SELECT 'Tables created: 14' AS info;
SELECT 'Sample data loaded for all tables' AS status;
